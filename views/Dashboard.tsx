...
const Dashboard: React.FC<DashboardProps> = ({ user, tasks, setScreen, onPomodoroComplete, doNotDisturb, setDoNotDisturb }) => {
  const [tip, setTip] = useState<string>('Loading your cosmic wisdom...');
  ...
  useEffect(() => {
    const fetchTip = async () => {
      const dailyTip = await generateDailyTip(user.learningStyle);
      setTip(dailyTip);
    };
    fetchTip();
  ...
  return (
    <div className="p-6 pt-10 pb-24 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-slate-400 text-sm uppercase tracking-wider">Good Morning,</h2>
        ...
        >
             <img src="https://picsum.photos/200" alt="Profile" className="w-full h-full rounded-full object-cover border-2 border-space-900" />
        </button>
      </div>
      ...
      <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-6 shadow-xl relative overflow-hidden group">
        ...
        <div className="grid grid-cols-2 gap-4">
             ...
             <div className="bg-space-800/50 p-4 rounded-2xl border border-white/5">
                <span className="text-slate-400 text-xs">Study Time</span>
        ...
        <div className="mt-6">
            <h4 className="text-sm font-medium text-slate-300 mb-2">AI Suggestion</h4>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
            ...
            <span className="font-medium text-sm">Style Test</span>
        </button>
      </div>
               ...
               <div className="bg-space-900 rounded-3xl p-4 flex items-center justify-between group-hover:bg-space-800 transition-colors">
                   <div className="flex items-center gap-4">
                       ...
                       <div className="text-left">
                           <p className="font-bold">Start {nextTask.title}</p>
                           <p className="text-xs text-slate-400">Next up in your plan</p>
                       </div>
                   </div>
       ...
       {showPomodoro && (
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-6">
           <div className="bg-space-800 border border-white/10 w-full max-w-sm rounded-3xl p-8 flex flex-col items-center relative">
             ...
             <p className="text-slate-400 text-sm mb-8">Stay focused for 25 minutes</p>
